...
Boneyard turns selected 3D BVH captures into readable clips for the eleven-bone SVG rig. The
raw capture is build-time input; a consuming lab receives only the generated catalog, and what
it does with a clip's timing — warping it onto a move, looping it, ignoring it — is the lab's
...
published Blender script is a visualization aid, not a retargeter. Boneyard follows the same
...
legible and checks those names against each BVH filename. [`catalog/clips.json`](../catalog/clips.json)
...
Nothing in the capture holds a prop and no clip draws one, but the grip is measurable: the two
...
Which way the file draws the rig is measured rather than assumed. The exporter writes its files
...
edit survives. `npm run build:motions` folds the directory into
[`catalog/clips.json`](../catalog/clips.json), and `npm run check:motions` fails when the two
disagree.
...
the fighter rig and claims no other origin. An authored clip that keeps a manifest clip's key
...
An imported clip carries its origin in `origins`, so a lab can inherit the presentation
decisions made for the clip it came from — arm layering falls back to the origin's depth
profile — and can list a clip no moveset claims as an authored variant of something known.
What a lab does with that is the lab's business; the catalog only records where the clip came
from.
