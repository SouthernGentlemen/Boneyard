...
export function rulesetPolicies(expected) {
  const main = expected.mainProtection;
  const tags = expected.releaseTags;
  return {
    main: {
      name: main.rulesetName,
      target: "branch",
      enforcement: main.protected ? "active" : "disabled",
      include: [`refs/heads/${expected.defaultBranch}`],
      exclude: [],
      bypassActors: main.bypassActors,
      requireBranchUpToDate: main.requireBranchUpToDate,
      requireExtraApprovalForUnattributedChanges: main.requireExtraApprovalForUnattributedChanges,
      requiredReviewThreadResolution: main.requiredReviewThreadResolution,
      doNotEnforceOnCreate: main.doNotEnforceOnCreate,
      requiredRules: [
        ...(!main.allowDeletion ? ["deletion"] : []),
        ...(!main.allowForcePush ? ["non_fast_forward"] : []),
        ...(main.requirePullRequest ? ["pull_request"] : []),
        "required_status_checks",
      ],
      requiredStatusChecks: expected.requiredStatusChecks.map((check) => check.name),
    },
    releaseTags: {
      name: tags.rulesetName,
      target: "tag",
      enforcement: tags.enforcement,
      include: [`refs/tags/${tags.pattern}`],
      exclude: [],
      bypassActors: tags.bypassActors,
      requiredRules: [
        ...(!tags.allowDeletion ? ["deletion"] : []),
        ...(!tags.allowUpdate ? ["update"] : []),
      ],
    },
  };
}
...
export function compareGithubRepositorySettings(expected, repository, rulesets) {
  ...
  for (const [key, policy] of Object.entries(rulesetPolicies(expected))) {
