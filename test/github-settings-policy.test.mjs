...
const policies = rulesetPolicies(expected);
...
function matchingActual() {
  return {
    ...
    rulesets: Object.values(policies).map((policy, index) => ({ id: index + 1, ...rulesetPayload(expected, policy) })),
  };
}
...
function main(actual) {
  return actual.rulesets.find((ruleset) => ruleset.name === policies.main.name);
}

function tag(actual) {
  return actual.rulesets.find((ruleset) => ruleset.name === policies.releaseTags.name);
}
...
test("required exact-head checks and current-main enforcement cannot weaken", () => {
  assert.match(failuresFor((actual) => { rule(main(actual), "required_status_checks").parameters.required_status_checks = []; }), /main required checks/);
...
test("missing token and read/write permission failures are distinct", async () => {
  await assert.rejects(githubApi("/repos/SouthernGentlemen/Boneyard"), { code: "GITHUB_AUTH_REQUIRED" });
  const denied = async () => new Response(JSON.stringify({ message: "denied" }), { status: 403 });
  await assert.rejects(githubApi("/repos/SouthernGentlemen/Boneyard", { token: "fixture", fetchImpl: denied }),
    { code: "GITHUB_ADMIN_READ_DENIED" });
  await assert.rejects(githubApi("/repos/SouthernGentlemen/Boneyard", { token: "fixture", method: "PATCH", fetchImpl: denied }),
